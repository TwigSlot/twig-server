...
class Resource(Node):
    ...
    def set_project(self, project: Project) -> Optional[Relationship]:
        ...
        self.project_rls = Relationship(
            self.conn, a_id=project_id, b_id=resource_id
        )
    ...
    def find_rls_with(self, tag: Tag):
        queryStr = (
            f"MATCH (n:{Resource._label_name})-[e]->(t:{Tag._label_name})"
    ...
    def update_all_positions(
        cls, db_conn: Neo4jConnection, new_positions: dict, project_id: int
    ) -> None:
    ...
    def get_tagged_resources(cls, db_conn: Neo4jDriver, tag: Tag):
        queryStr = f"MATCH (n:{Resource._label_name})\
    ...
    def list_resource_tags(
        cls, db_conn: Neo4jDriver, resource: Any
    ) -> List[Record]:
        """
